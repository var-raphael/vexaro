...
var standaloneURLPattern = regexp.MustCompile(`(?i)^https?://\S+$`)

func compressContent(content string, includeLinks bool) string {
	...
	content = stripLeadingNav(content, includeLinks)
	content = deduplicateWindows(content)
	content = stripFragments(content, includeLinks)
...
func stripLeadingNav(content string, includeLinks bool) string {
	...
	for i := 0; i < len(words); i++ {
		if includeLinks && standaloneURLPattern.MatchString(words[i]) {
...
func stripFragments(content string, includeLinks bool) string {
	...
	for _, part := range parts {
		...
		if includeLinks && standaloneURLPattern.MatchString(part) {
