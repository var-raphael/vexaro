...
func layer2(targetURL, html string) (string, error) {
	...
	if nextData := extractNextFChunks(html); nextData != "" {
		return nextData, nil
	}
	...
	if err == nil {
		...
		if isWordPress(doc) {
			...
			if slug != "" {
				...
				for _, ep := range endpoints {
					...
					if result := probeEndpoint(ep); result != "" {
						return result, nil
					}
				}
			}
		...
		if alternateURL != "" && alternateURL != targetURL {
			jitterBetween(0.3, 1.0)
			if result := probeEndpoint(alternateURL); result != "" {
				return result, nil
			}
		}
	...
	if len(endpoints) > 0 {
		if result := probeEndpointsParallel(endpoints); result != "" {
			return result, nil
		}
	}
...
// extractNextFChunks collects self.__next_f.push([...]) chunks injected by
// the Next.js 13+ app router and reassembles them into a single JSON string.
func extractNextFChunks(html string) string {
	...
	for _, m := range matches {
		...
		// payload is a JSON string value — unwrap the outer quotes
	...
	// The combined payload is RSC wire format — find the first JSON object or array
	for i, ch := range combined {
		if ch == '{' || ch == '[' {
			...
			// try to extract balanced JSON from the start
...
// extractEmbeddedState handles both { object } and [ array ] root values.
...
// extractBalanced extracts a balanced JSON object or array starting at s[0].
...
func probeEndpointsParallel(endpoints []string) string {
	...
	// return the first result that comes back (lowest latency wins)
	// but prefer lower index (more specific endpoints) if multiple arrive
