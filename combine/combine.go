	...
	for _, key := range datasetOrder {
		...
		for _, job := range jobs {
			...
			if _, err := os.Stat(extractPath); os.IsNotExist(err) {
				log.Printf("[combine] missing extract.json queue_id=%d path=%s — marking failed", job.QueueID, extractPath)
			...
			if err != nil {
				log.Printf("[combine] load error queue_id=%d path=%s: %v — marking failed", job.QueueID, extractPath, err)
				failedQueueIDs = append(failedQueueIDs, job.QueueID)
				continue
			}
			...
			if sourceURL != "" {
				source := map[string]interface{}{"url": sourceURL}
...
// CombineFromPaths reads paths from a file and combines — kept for dev/debug.
func CombineFromPaths(pathsFile string, datasetFolder string) error {
	...
	datasetID, err := ResolveDatasetID(datasetFolder)
	if err != nil {
		return fmt.Errorf("resolve dataset id: %w", err)
	}
	...
	for _, rawPath := range paths {
		...
		if raw, ok := data["entities"].([]interface{}); ok {
			for _, e := range raw {
				if entity, ok := e.(map[string]interface{}); ok {
					entity["_source"] = sourceURL
					entities = append(entities, entity)
	...
	if len(entities) == 0 {
		log.Printf("[combine] no entities extracted — skipping version save")
		return nil
	}

	if err := versioning.SaveVersion(datasetID, datasetFolder, entities, sources); err != nil {
		return fmt.Errorf("save version: %w", err)
	}

	log.Printf("[combine] done — %d entities, %d sources", len(entities), len(sources))
	return nil
}

// ResolveDatasetID parses dataset_id from folder name (format: data_name-dataset_id).
...
// ResolveDatasetFolder looks up dataset_id for apiName and returns the folder key.
