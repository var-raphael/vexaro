...
func setupMCPServer() *server.MCPServer {
	...
	s.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			err := db.Get().QueryRow(`
				SELECT user_id, COALESCE(alias, data_name), dataset_type, active_version, include_links
				FROM datasets WHERE dataset_id = ?
			`, datasetID).Scan(&ownerID, &dataName, &datasetType, &activeVersion, &includeLinks)
			if err == sql.ErrNoRows {
